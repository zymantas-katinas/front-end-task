...
function PasswordsList() {
    ...
    const sortCount = () => {
      ...
      if(accending) {
        ...
        setAccending(false)
        setRotate("passwords__arrow rotate")
      } else {
        setPasswords(sortedDesc)
        setAccending(true)
    ...
    const allPasswords = passwords.slice(0, showItems).map((data) => 
      <div key = {data.value} ><hr /><ListItem id = {sortedPasswords.indexOf(data) + 1} value = {data.value} count ={data.count} /></div>
    )
    ...
    return (
        <div className ="passwords__list">
           ...
           <button className = "button-medium" onClick = {handleClick}>{buttonText}</button>
        </div>
    )
}
